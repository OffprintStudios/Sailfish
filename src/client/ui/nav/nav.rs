...
use crate::client::ui::util::Modal;

#[component]
pub fn Nav() -> impl IntoView {
    let (open_settings, set_open_settings) = create_signal(false);
    ...
    view! {
        ...
        >
            <nav class="flex items-center max-w-[90rem] h-[55px] md:h-[65px] w-full md:w-11/12 pl-4 pr-2 md:px-0 mx-auto">
                <div class="flex items-center w-1/3 text-white">
                    ...
                    <a
                        class="hidden md:block relative top-0.5 text-white ml-4 all-small-caps text-lg firefox:text-base border-b-2 border-transparent transition font-semibold font-header"
                    >
                        "Social"
                    </a>
                </div>
                ...
                <div class="w-1/3 flex items-center" style="font-family: var(--header-text);">
                    ...
                    <button
                        class="flex items-center text-white px-3 py-2 firefox:text-sm rounded-xl transition hover:bg-zinc-300/25 hover:backdrop-blur"
                        on:click=move |_| {
                            set_open_settings(true);
                        }
                    >
                        <span><Icon icon=remixicon::RiSettings5SystemLine width="20px" height="20px" /></span>
                    </button>
        ...
        <Modal id="settings-modal".to_string() open=open_settings set_open=set_open_settings>
            "hi hello"
        </Modal>
    }
}