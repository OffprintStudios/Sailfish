...
impl Account {
    ...
    pub async fn new(email: String, password: String, db: &Pool<Postgres>) -> Result<Self, AppError> {
    ...
    pub async fn verify(email: String, password: String, db: &Pool<Postgres>) -> Result<Self, AppError> {
        let clean_email = clean(&email);
        let clean_password = clean(&password);
    ...
    pub async fn fetch_by_email(email: String, db: &Pool<Postgres>) -> Option<Account> {
        let clean_email = clean(&email);
    ...
    pub async fn fetch_by_id(id: String, db: &Pool<Postgres>) -> Option<Account> {
        sqlx::query_as!(
            Self,
            r#"
                SELECT
    ...
    pub async fn confirm(&self, db: &Pool<Postgres>) -> Result<(), AppError> {
        _ = sqlx::query!(
            r#"
    ...
    pub async fn reset_password(&self, new_password: String, db: &Pool<Postgres>) -> Result<(), AppError> {
