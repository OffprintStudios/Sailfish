...
impl Account {
    ...
    pub async fn new(email: String, password: String, db: &Pool<Postgres>) -> Result<Self, SailfishError> {
    ...
    pub async fn verify_credentials(email: String, password: String, db: &Pool<Postgres>) -> Result<Account, SailfishError>{
        let clean_email = clean(&email);
        let clean_password = clean(&password);
    ...
    pub async fn fetch_by_id(id: Uuid, db: &Pool<Postgres>) -> Option<Account> {
        sqlx::query_as!(
            Self,
            r#"
                SELECT
