...
pub fn Nav() -> impl IntoView {
    let (current_user, set_current_user) = create_signal::<Option<Profile>>(None);
    
    create_effect(move |_| {
        let (auth, _, _) = use_local_storage::<AuthState, JsonCodec>("auth");
        set_current_user(auth().current_profile);
    });
            ...
            <nav class="flex items-center max-w-[90rem] h-[65px] w-full md:w-11/12 pl-4 pr-2 md:px-0 mx-auto">
                ...
                <div class="w-1/3 flex items-center" style="font-family: var(--header-text);">
                    ...
                    >
                        ...
                        <Guide profile=current_user().unwrap() />
                    </Show>
                </div>
            </nav>
        </div>
