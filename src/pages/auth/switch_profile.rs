...
pub async fn load_profiles() -> Result<Vec<ProfileObject>, AppError> {
    ...
    let profiles = match Profile::fetch_owned(account.id, &state.db).await {
        Ok(profiles) => {
            let mut objects = Vec::<ProfileObject>::new();
            for profile in profiles {
                objects.push(profile.to_object(&state.db).await);
            }
            objects
        },
...
pub fn SwitchProfilePage() -> impl IntoView {
    ...
    let profiles = move || {
        Suspend::new(async move {
            load_profiles.await.map(|profiles| {
                ...
                if profiles.is_empty() {
                    ...
                    Either::Right(profiles.iter().map(move |profile| {
                        let p = profile.clone();
