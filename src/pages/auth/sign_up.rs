...
pub async fn sign_up(
    ...
    if Account::fetch_by_email(email.clone(), &state.db).await.is_some() {
        return Err(ServerFnError::new(SignUpError::Conflict));
    }

    let account = Account::new(email, password, &state.db).await?;
    let confirmation_code = ConfirmationCode::new(account.id, Utc::now() + Duration::seconds(3600), &state.db).await?;

    let new_email = Email {
        kind: EmailKind::ConfirmEmail,
        from: "Beatriz <beatriz@offprint.cafe>".into(),
        to: account.email.clone(),
        subject: "Welcome to Offprint!".into(),
        token: Some(confirmation_code.token)
    };
