...
pub async fn send_reset_code(email: String) -> Result<(), AppError> {
    ...
    if let Some(account) = Account::fetch_by_email(email, &state.db).await {
        let Extension(mut queue) = leptos_axum::extract::<Extension<RedisStorage<Email>>>().await.map_err(AppError::ServerFnError)?;
...
pub async fn reset_password(new_password: String, repeat_password: String, token: String) -> Result<(), AppError> {
    use crate::state::AppState;
