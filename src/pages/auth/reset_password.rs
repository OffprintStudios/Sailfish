...
pub async fn send_reset_code(email: String) -> Result<(), ServerFnError> {
    ...
    use crate::models::accounts::{Account, PasswordReset};
    ...
    if let Some(account) = Account::fetch_by_email(email, &state.db).await {
        ...
        let reset_code = PasswordReset::new(account.id, chrono::Utc::now() + chrono::Duration::seconds(3600), &state.db).await?;
...
pub async fn reset_password(new_password: String, repeat_password: String, token: String) -> Result<(), ServerFnError> {
    use crate::state::AppState;
    use crate::models::accounts::PasswordReset;
    ...
    if let Ok(valid_account) = PasswordReset::validate(token, &state.db).await {
