	...
	<div class="home-header">
		<div class="w-1/4">
			<div class="deco-box flex-col">
				<span class="relative top-2 left-[0.075rem] text-4xl">{today.toLocaleString('default', { month: 'short' })}</span>
				<span class="relative bottom-1.5 left-[0.075rem] text-xl">{today.getFullYear()}</span>
			</div>
		</div>
		<div class="w-2/4 flex flex-col items-center justify-center">
			...
			/>
			<h2 class="text-white font-light text-lg">
				{currSlogan}
			</h2>
		</div>
...
<style lang="scss">
	div.home-header {
		@apply text-white text-center my-6 rounded-xl px-4 py-2 w-full hidden lg:flex items-center justify-center;
