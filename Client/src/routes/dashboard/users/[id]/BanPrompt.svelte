<script lang="ts">
	...
	import { BanDuration } from "$lib/models/admin/users/durations";
	...
	const { form, errors, isSubmitting } = createForm({
		onSubmit: async (values) => {
			...
			const response = await postReq<void>(
				`/admin/ban-user?profileId=${$account.currProfile?.id}`,
				formInfo
			);
...
<div class="rounded-xl overflow-hidden bg-zinc-200 dark:bg-zinc-700 dark:highlight-shadowed">
	...
	<form class="px-4 pb-4 w-full lg:w-[400px]" use:form>
		<select
			...
			name="duration"
		>
			<option value={BanDuration.OneDay}>{BanDuration.OneDay}</option>
			<option value={BanDuration.ThreeDays}>{BanDuration.ThreeDays}</option>
			<option value={BanDuration.OneWeek}>{BanDuration.OneWeek}</option>
			<option value={BanDuration.TwoWeeks}>{BanDuration.TwoWeeks}</option>
			<option value={BanDuration.OneMonth}>{BanDuration.OneMonth}</option>
			<option value={BanDuration.ThreeMonths}>{BanDuration.ThreeMonths}</option>
			<option value={BanDuration.Permanent}>{BanDuration.Permanent}</option>
		</select>
		<TextArea
			title="Reason"
			name="reason"
