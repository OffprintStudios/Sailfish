<script lang="ts">
	...
	let hasFavorited: FavoriteBlog | null = null;
	...
	async function fetchFavorite() {
		...
		const response = await getReq<FavoriteBlog>(
			`/blogs/fetch-favorite/${blog.id}?profileId=${$account.currProfile?.id}`
		);
	...
	async function favoriteBlog() {
		loadingFavorite = true;
		const response = await postReq<FavoriteBlog>(
			`/blogs/add-favorite/${blog.id}?profileId=${$account.currProfile?.id}`,
			{}
		);
		if ((response as ResponseError).error) {
	...
	async function unfavoriteBlog() {
		const response = await delReq<void>(
			`/blogs/remove-favorite/${blog.id}?profileId=${$account.currProfile?.id}`
		);
		if ((response as ResponseError).error) {
			const error = response as ResponseError;
			toast.error(error.message);
	...
	const submitForm = createSubmitHandler({
		async onSubmit(values) {
			...
			const response = await patchReq<Blog>(
				`/api/content/blogs/${blog.id}/update-blog?profileId=${$account.currProfile?.id}`,
				formInfo
			);
			if ((response as ResponseError).error) {
				const error = response as ResponseError;
	...
	async function publishBlog(date?: Date) {
		...
		const response = await patchReq<Blog>(
			`/blogs/publish-blog/${blog.id}?profileId=${$account.currProfile?.id}`,
			pubBlog
		);
		if ((response as ResponseError).error) {
			const error = response as ResponseError;
	...
	async function deleteBlog() {
		openPopup(DeleteBlogPrompt, {
			async onConfirm() {
				const response = await delReq<void>(
					`/blogs/delete-blog/${blog.id}?profileId=${$account.currProfile?.id}`
				);
				if ((response as ResponseError).error) {
					const error = response as ResponseError;
					toast.error(error.message);
...
<div class="mx-auto max-w-4xl flex flex-col my-12 md:flex-row">
	<div class="hidden md:flex flex-row md:flex-col md:max-w-[108.16px] md:min-w-[108.16px]">
		<div class="date-block bg-zinc-200 dark:bg-zinc-700 dark:highlight-shadowed">
			...
			{#if blog.publishedOn}
				<span class="month-and-day">
					{new Date(blog.publishedOn).toLocaleString("default", { month: "short" })}
					{new Date(blog.publishedOn).getDate()}
				</span>
						...
						<Dropdown position="right-start">
							...
							<svelte:fragment slot="items">
								...
								<button
									type="button"
									on:click={() =>
										toast.error("This feature is not yet supported.")}
								>
		...
		{/if}
		<div
			class="flex flex-col rounded-xl bg-zinc-200 dark:bg-zinc-700 dark:highlight-shadowed p-2"
		>
	...
	<div class="blog-container">
		{#if isEditing}
			<form use:form>
				<TextField name="title" type="text" title="Title" placeholder="Just Catching Up" />
				<div class="my-4" />
		...
		{:else}
			...
			<div class="flex md:hidden items-center text-xs mb-4">
				...
				<span
					>{abbreviate(blog.stats.comments)} comment{pluralize(blog.stats.comments)}</span
				>
				<span class="mx-1">•</span>
			...
			{#if blog.editedOn}
				<span class="text-xs italic border-t pt-4"
					>Last edited <Time timestamp={blog.editedOn} relative="true" /></span
				>
			{/if}
		{/if}
	</div>
