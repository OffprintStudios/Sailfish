<script lang="ts" context="module">
	...
	export const load: Load = async ({ params, fetch }) => {
		const response = await fetch(`/api/profiles/fetch-profile/${params.id}`);
...
<script lang="ts">
	...
	import { Home5Line, CupLine, QuillPenLine, BarChart2Line, ImageEditLine, ServiceLine, Flag2Line, CloseCircleLine } from "svelte-remixicon";
...
<svelte:head>
	<title>{profile.username}'s Profile &mdash; Offprint</title>
	<!-- Primary Meta Tags -->
	<meta name="title" content="{profile.username}'s Profile" />
	<meta name="description" content="{profile.info.bio}" />

	<!-- Open Graph / Facebook -->
	<meta property="og:type" content="website" />
	<meta property="og:url" content="https://offprint.net/profile/{profile.id}/{slugify(profile.username)}" />
	<meta property="og:title" content="{profile.username}'s Profile" />
	<meta
		property="og:description"
		content="{profile.info.bio}"
	/>
	<meta property="og:image" content="{profile.avatar}" />

	<!-- Twitter -->
	<meta property="twitter:card" content="summary_large_image" />
	<meta property="twitter:url" content="https://offprint.net/profile/{profile.id}/{slugify(profile.username)}" />
	<meta property="twitter:title" content="{profile.username}'s Profile" />
	<meta
...
<div class="w-11/12 mx-auto max-w-7xl mb-6">
	<div class="profile-nav-container">
		{#if profile.info.coverPic}
			<div class="profile-cover">
				<img src={profile.info.coverPic} alt="{profile.username}'s Cover Picture" />
			</div>
		{:else}
		...
		<div class="profile-nav bg-zinc-200 dark:bg-zinc-700">
			<Avatar src={profile.avatar} size="150px" borderWidth="2px" />
			<div class="ml-4 self-center">
				<div class="flex items-center">
					<RoleBadge roles={profile.account.roles} />
					<h3>{profile.username}</h3>
				</div>
				<div class="flex items-center text-xs">
					<a class="text-zinc-800 dark:text-white no-underline">0 followers</a>
					<span class="text-zinc-800 dark:text-white mx-1">•</span>
					<a class="text-zinc-800 dark:text-white no-underline">0 following</a>
				</div>
			</div>
			<div class="flex-1"><!--spacer--></div>
			<a
				class="link hover:no-underline hover:bg-zinc-300"
				href="/profile/{profile.id}/{slugify(profile.username)}"
				class:active={$page.url.pathname === `/profile/${profile.id}/${slugify(profile.username)}`}
			>
			...
			<a
				...
				class:active={$page.url.pathname === `/profile/${profile.id}/${slugify(profile.username)}/blogs`}
			>
			...
			<a
				...
				class:active={$page.url.pathname === `/profile/${profile.id}/${slugify(profile.username)}/works`}
			>
			...
			<a
				class="link hover:no-underline hover:bg-zinc-300"
				href="/profile/{profile.id}/{slugify(profile.username)}/shelves"
				class:active={$page.url.pathname === `/profile/${profile.id}/${slugify(profile.username)}/shelves`}
			>
			...
			<button class="link hover:bg-zinc-300">
...
<style lang="scss">
	div.profile-nav-container {
		...
		div.profile-nav {
			@apply flex items-end px-2 py-2 rounded-b-xl h-[77px] relative z-10;
			h3 {
				@apply text-4xl;
			}
			a.link, button.link {
