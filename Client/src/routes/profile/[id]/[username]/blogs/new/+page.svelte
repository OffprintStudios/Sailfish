<script lang="ts">
	...
	const { form, data, errors, isSubmitting } = createForm<BlogForm>({
		async onSubmit(values) {
			const response = await postReq<Blog>(`/blogs/create-blog?profileId=${$account.currProfile.id}`, values);
			if ((response as ResponseError).error) {
				const error = response as ResponseError;
				toast.error(error.message);
			} else {
				const blog = response as Blog;
				await goto(`/profile/${profile.id}/${slugify(profile.username)}/blog/${blog.id}/${slugify(blog.title)}`);
			}
		},
		validate(values) {
			const errors = {
	...
	<form class="max-w-4xl mx-auto rounded-xl overflow-hidden flex flex-col bg-zinc-200 dark:bg-zinc-700 dark:highlight-shadowed" use:form>
		...
		<div class="flex border-b border-zinc-400 dark:border-zinc-500">
			<div class="w-1/4 flex flex-col items-end px-4 pt-2 pb-3 border-r border-zinc-400 dark:border-zinc-500">
				...
				<span class="text-xs italic text-zinc-400 text-right">What's the name of your blog?</span>
			</div>
		...
		<div class="flex border-b border-zinc-400 dark:border-zinc-500">
			<div class="w-1/4 flex flex-col items-end px-4 pt-2 pb-3 border-r border-zinc-400 dark:border-zinc-500">
				...
				<span class="text-xs italic text-zinc-400 text-right">What's the gist of your post? Don't worry, this part's optional.</span>
			</div>
			<div class="w-3/4 px-2.5 flex flex-col items-center justify-center">
				<TextField
					name="desc"
		...
		<div class="flex border-b border-zinc-400 dark:border-zinc-500">
			<div class="w-1/4 flex flex-col items-end px-4 pt-2 pb-3 border-r border-zinc-400 dark:border-zinc-500">
				...
				<span class="text-xs italic text-zinc-400 text-right">
					Check out the Offprint Omnibus for info about which one would be appropriate for your blog.
				</span>
			</div>
