<script lang="ts">
	...
	import { Editor, TextField, TextArea } from "$lib/ui/forms";
	...
	const { form, errors, data, isSubmitting } = createForm({
		async onSubmit(values) {
			...
			const response = await postReq<Work>(
				`/works/create-work?profileId=${$account.currProfile?.id}`,
				values as WorkForm
			);
			if ((response as ResponseError).error) {
				const error = response as ResponseError;
...
{#if $account.account && $account.currProfile && $account.currProfile.id === profile.id}
	...
	>
		<div class="flex items-center justify-center px-2 py-4" style="background: var(--accent);">
			...
			<h1 class="text-white text-2xl lg:text-4xl">Create a New Work</h1>
		</div>
