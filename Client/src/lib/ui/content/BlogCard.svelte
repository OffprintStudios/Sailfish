<script lang="ts">
    import { RiArrowUpSLine, RiArrowDownSLine, RiDiscussLine, RiEyeOffLine } from "svelte-remixicon";
...
>
    <div
        class="absolute top-0 right-0 z-10 all-small-caps text-white px-2 rounded-bl-xl font-bold"
    ...
    {#if blog.rating === ContentRating.mature || blog.rating === ContentRating.explicit}
    ...
    {/if}
    <div class="blog-body text-sm p-6 h-[calc(256px-60px)] relative blur" class:blur={blog.rating === ContentRating.mature || blog.rating === ContentRating.explicit}>
        {@html blog.body}
    </div>
    <div class="flex items-center w-full relative z-10 h-[60px]" style="background: rgb(var(--accent));">
        ...
        <div class="flex flex-col w-full" style="font-family: var(--header-text);">
            ...
            <div class="flex items-center w-full relative -top-1 font-bold text-white/50 all-small-caps pr-3">
                <span class="text-lg">{localeDate(blog.createdAt, "longDate")}</span>
