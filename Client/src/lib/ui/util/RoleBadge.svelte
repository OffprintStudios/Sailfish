<script lang="ts">
	...
	function determineProminentRole() {
		...
		const hasMaintainer = intersection<Roles>(roles, [Roles.Maintainer]);
		...
		const hasVIP = intersection<Roles>(roles, [Roles.VIP]);
		const hasSupporter = intersection<Roles>(roles, [Roles.Supporter]);
