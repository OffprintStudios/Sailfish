    ...
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.create_table(
            Table::create()
                .table(Account::Table)
                .if_not_exists()
                .col(ColumnDef::new(Account::Id).uuid().not_null().primary_key().default(Expr::cust("gen_random_uuid()")))
                .col(ColumnDef::new(Account::Email).string().not_null().unique_key())
                .col(ColumnDef::new(Account::Password).string().not_null())
                .col(ColumnDef::new(Account::Roles).array(ColumnType::String(None)).not_null())
                .col(ColumnDef::new(Account::TermsAgree).boolean().not_null().default(false))
