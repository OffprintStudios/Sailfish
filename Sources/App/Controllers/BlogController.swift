...
struct BlogController: RouteCollection {
    func boot(routes: RoutesBuilder) throws {
        ...
        ).get("all") { request async throws -> Page<BlogView> in
        ...
        blogs.grouped(
                ...
                SessionToken.guardMiddleware(),
                IdentityGuard(needs: [.user], checkProfile: true)
        ).post("create") { request async throws -> HTTPResponseStatus in
            let profile = request.user!.profile!
        ...
        blogsWithId.grouped(
                ...
                SessionToken.guardMiddleware(),
                IdentityGuard(needs: [.user], checkProfile: true)
        ).patch("update") { request async throws -> HTTPResponseStatus in
            let profile = request.user!.profile!
            let id = request.parameters.get("id")!
            let formInfo = try request.content.decode(Blog.FormInfo.self)
        ...
        blogsWithId.grouped(
                ...
                SessionToken.guardMiddleware(),
                IdentityGuard(needs: [.user], checkProfile: true)
        ).delete("delete") { request async throws -> HTTPResponseStatus in
            let profile = request.user!.profile!
            let id = request.parameters.get("id")!
