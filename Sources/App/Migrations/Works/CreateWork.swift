...
struct CreateWork: AsyncMigration {
    func prepare(on database: Database) async throws {
        return try await database.schema("works")
            ...
            .field("words", .uint64, .required, .sql(.default(0)))
            .field("views", .uint64, .required, .sql(.default(0)))
            .field("likes", .uint64, .required, .sql(.default(0)))
            .field("dislikes", .uint64, .required, .sql(.default(0)))
            .field("comments", .uint64, .required, .sql(.default(0)))
